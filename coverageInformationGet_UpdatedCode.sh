...
for benchmark in * ; do
	...
	if [ -d $benchmark ]; then
		...
		while read -r testinput; do
			...
			echo "gcov -j -b -c $benchmark" >> ../$coveragefile
      			echo "gunzip $benchmark.gcov.json.gz" >>../$coveragefile
      			echo "rm $benchmark.gcov.json.gz" >> ../$coveragefile
			# move coverage file to the coverage directory
			echo "mv $benchmark.gcov.json coverage/$test_num" >> ../$coveragefile
		...
		while [ -d "v$i" ]; do
    		cd "v$i"
    		# run replace and totinfo with -lm flag
    		if [ "$benchmark" = "totinfo" ] || [ "$benchmark" = "replace" ]; then
        		gcc -g -o "$benchmark" "$benchmark.c" -lm -Wno-return-type -fprofile-arcs -ftest-coverage
    		else
        		gcc -g -o "$benchmark" "$benchmark.c" -Wno-return-type -fprofile-arcs -ftest-coverage
    		fi
